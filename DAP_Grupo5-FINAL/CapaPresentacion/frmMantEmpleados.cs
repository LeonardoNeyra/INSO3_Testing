    ...
    public partial class frmMantEmpleados : Form
    {
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            if (existeE)
            {
                if (this.txtNombres.Text == "" || this.txtApellidos.Text == "" || this.txtDireccion.Text == "" || this.txtDNI.TextLength != 8 || this.cmbCategoria.SelectedIndex == -1 || this.cmbVentanilla.SelectedIndex == -1)
                {
                    MessageBox.Show("Campos Faltantes o Datos Erróneos", "Error al Modificar Empleado", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    NEmpleado oEneg = new NEmpleado();
        ...
        private void txtNombres_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if (System.Text.RegularExpressions.Regex.IsMatch(txtNombres.Text, @"^[a-zA-Z]+$"))
            {
                e.Handled = false;
            }
            else if (char.IsControl(e.KeyChar))
        ...
        {
            /*if ((e.KeyChar >= 0 && e.KeyChar <= 64) || (e.KeyChar >= 91 && e.KeyChar <= 96) || (e.KeyChar >= 123 && e.KeyChar <= 126))
            {
                e.Handled = false;
            }*/
            if (char.IsLetter(e.KeyChar))
